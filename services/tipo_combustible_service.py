# services/tipo_combustible_service.py
"""
Servicio para TipoCombustible.
...
class TipoCombustibleService(BaseService):
    """Servicio CRUD completo para TipoCombustible"""
    ...
    def _validar_codigo_unico(self, codigo: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoCombustible WHERE Codigo = %s"
        params = [codigo]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de combustible con el código '{codigo}'"}
    ...
    def _validar_nombre_unico(self, nombre: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoCombustible WHERE Nombre = %s"
        params = [nombre]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de combustible con el nombre '{nombre}'"}
    ...
    def _validar_tiene_vehiculos_asociados(self, id_registro: int) -> bool:
        """Verifica si hay vehículos activos con este tipo de combustible"""
        query = """
            SELECT COUNT(*) AS Total
            FROM Vehiculo
            WHERE IdTipoCombustible = %s AND Eliminado = 0
        """
        resultado = self.db.select(query, (id_registro,))
    ...
    def crear(self, datos: Dict) -> Dict:
        ...
        try:
            ...
            query_id = "SELECT MAX(Id) AS NuevoId FROM TipoCombustible WHERE Codigo = %s AND Eliminado = 0"
            ...
            logger.error(f"Error al crear TipoCombustible: {e}")
    ...
    def obtener_por_id(self, id_registro: int) -> Dict:
        """Obtiene un tipo de combustible por su ID"""
        try:
            registro = self._validar_existencia(id_registro)

            if not registro:
                return {"error": f"Tipo de combustible con Id {id_registro} no encontrado", "status": 404}

            return {"data": registro}

        except Exception as e:
            logger.error(f"Error al obtener TipoCombustible {id_registro}: {e}")
    ...
    def listar(
        ...
        """Lista tipos de combustible con paginación y filtros"""
        try:
            if pagina < 1:
                pagina = 1
            if por_pagina < 1:
            ...
            # Contar total
            count_query = f"SELECT COUNT(*) AS Total FROM TipoCombustible WHERE {where_sql}"
            ...
            logger.error(f"Error al listar TipoCombustible: {e}")
    ...
    def listar_todos(
        ...
        try:
            ...
            logger.error(f"Error al listar todos TipoCombustible: {e}")
            return {"error": f"Error interno al listar registros: {str(e)}", "status": 500}
    ...
    def actualizar(self, id_registro: int, datos: Dict) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error al actualizar TipoCombustible {id_registro}: {e}")
    ...
    def eliminar(self, id_registro: int) -> Dict:
        """
        Eliminación lógica de un tipo de combustible.
        ...
        try:
            ...
            if self._validar_tiene_vehiculos_asociados(id_registro):
                return {
                    "error": "No se puede eliminar este tipo de combustible porque tiene vehículos activos asociados",
                    "status": 409
                }
            ...
            return {"data": {"mensaje": f"Tipo de combustible '{existente['Nombre']}' eliminado exitosamente"}}

        except Exception as e:
            logger.error(f"Error al eliminar TipoCombustible {id_registro}: {e}")
