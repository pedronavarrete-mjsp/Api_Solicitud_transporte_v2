# services/tipo_licencia_service.py
"""
Servicio para TipoLicencia.
...
class TipoLicenciaService(BaseService):
    """Servicio CRUD completo para TipoLicencia"""
    ...
    def _validar_codigo_unico(self, codigo: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoLicencia WHERE Codigo = %s"
        params = [codigo]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de licencia con el código '{codigo}'"}
    ...
    def _validar_nombre_unico(self, nombre: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoLicencia WHERE Nombre = %s"
        params = [nombre]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de licencia con el nombre '{nombre}'"}
    ...
    def _validar_tiene_motoristas_asociados(self, id_registro: int) -> bool:
        """Verifica si hay motoristas activos con este tipo de licencia"""
        query = """
            SELECT COUNT(*) AS Total
            FROM DetallePerfilMotorista
            WHERE IdTipoLicencia = %s AND Eliminado = 0
        """
        resultado = self.db.select(query, (id_registro,))
    ...
    def crear(self, datos: Dict) -> Dict:
        """Crea un nuevo tipo de licencia con validación de duplicados y reactivación"""
        try:
            ...
            query_insert = """
                INSERT INTO TipoLicencia
                    (Codigo, Nombre, Eliminado, FechaHoraCreacion, FechaHoraActualizacion)
                VALUES (%s, %s, 0, %s, %s)
            """
            self.db.ejecutar(query_insert, (codigo, nombre, ahora, ahora), fetch=False, as_dict=False)

            query_id = "SELECT MAX(Id) AS NuevoId FROM TipoLicencia WHERE Codigo = %s AND Eliminado = 0"
            ...
            logger.error(f"Error al crear TipoLicencia: {e}")
    ...
    def obtener_por_id(self, id_registro: int) -> Dict:
        """Obtiene un tipo de licencia por su ID"""
        try:
            registro = self._validar_existencia(id_registro)
            if not registro:
                return {"error": f"Tipo de licencia con Id {id_registro} no encontrado", "status": 404}
            return {"data": registro}
        except Exception as e:
            logger.error(f"Error al obtener TipoLicencia {id_registro}: {e}")
    ...
    def listar(self, pagina: int = 1, por_pagina: int = 10,
        ...
        """Lista tipos de licencia con paginación y filtros"""
        try:
            ...
            count_query = f"SELECT COUNT(*) AS Total FROM TipoLicencia WHERE {where_sql}"
            ...
            logger.error(f"Error al listar TipoLicencia: {e}")
    ...
    def listar_todos(self, codigo: Optional[str] = None, nombre: Optional[str] = None,
                     busqueda: Optional[str] = None) -> Dict:
        """Lista todos los tipos de licencia activos sin paginación"""
        try:
            where_clauses = ["Eliminado = 0"]
            params = []
            ...
            logger.error(f"Error al listar todos TipoLicencia: {e}")
            return {"error": f"Error interno al listar registros: {str(e)}", "status": 500}
    ...
    def actualizar(self, id_registro: int, datos: Dict) -> Dict:
        ...
        try:
            ...
            query = f"UPDATE TipoLicencia SET {', '.join(set_parts)} WHERE Id = %s AND Eliminado = 0"
            self.db.ejecutar(query, tuple(params), fetch=False, as_dict=False)

            registro = self._validar_existencia(id_registro)
            return {"data": registro}
        except Exception as e:
            logger.error(f"Error al actualizar TipoLicencia {id_registro}: {e}")
    ...
    def eliminar(self, id_registro: int) -> Dict:
        """Eliminación lógica de un tipo de licencia"""
        try:
            existente = self._validar_existencia(id_registro)
            if not existente:
                return {"error": f"Tipo de licencia con Id {id_registro} no encontrado", "status": 404}
            ...
            if self._validar_tiene_motoristas_asociados(id_registro):
                return {
                    "error": "No se puede eliminar este tipo de licencia porque tiene motoristas activos asociados",
                    "status": 409
                }
            ...
            return {"data": {"mensaje": f"Tipo de licencia '{existente['Nombre']}' eliminado exitosamente"}}
        except Exception as e:
            logger.error(f"Error al eliminar TipoLicencia {id_registro}: {e}")
