# services/tipo_disponibilidad_service.py
"""
Servicio para TipoDisponibilidad.
...
class TipoDisponibilidadService(BaseService):
    """Servicio CRUD completo para TipoDisponibilidad"""

    TABLA = "TipoDisponibilidad"
    ...
    def _validar_codigo_unico(self, codigo: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoDisponibilidad WHERE Codigo = %s"
        params = [codigo]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de disponibilidad con el código '{codigo}'"}
    ...
    def _validar_nombre_unico(self, nombre: str, excluir_id: Optional[int] = None) -> Dict:
        ...
        query = "SELECT Id, Eliminado FROM TipoDisponibilidad WHERE Nombre = %s"
        params = [nombre]
        ...
        if registro["Eliminado"]:
            ...
            return {"disponible": False, "error": f"Ya existe un tipo de disponibilidad con el nombre '{nombre}'"}
    ...
    def _validar_tiene_motoristas_asociados(self, id_registro: int) -> bool:
        """Verifica si hay motoristas activos con este tipo de disponibilidad"""
        query = """
            SELECT COUNT(*) AS Total
            FROM DetallePerfilMotorista
            WHERE IdTipoDisponibilidad = %s AND Eliminado = 0
        """
        resultado = self.db.select(query, (id_registro,))
    ...
    def crear(self, datos: Dict) -> Dict:
        ...
        try:
            ...
            query_insert = """
                INSERT INTO TipoDisponibilidad
                    (Codigo, Nombre, Eliminado,
                     FechaHoraCreacion, FechaHoraActualizacion)
                VALUES (%s, %s, 0, %s, %s)
            """
            params = (codigo, nombre, ahora, ahora)

            self.db.ejecutar(query_insert, params, fetch=False, as_dict=False)

            # Obtener ID del registro creado
            query_id = "SELECT MAX(Id) AS NuevoId FROM TipoDisponibilidad WHERE Codigo = %s AND Eliminado = 0"
            ...
            logger.error(f"Error al crear TipoDisponibilidad: {e}")
    ...
    def obtener_por_id(self, id_registro: int) -> Dict:
        """Obtiene un tipo de disponibilidad por su ID"""
        try:
            registro = self._validar_existencia(id_registro)

            if not registro:
                return {"error": f"Tipo de disponibilidad con Id {id_registro} no encontrado", "status": 404}

            return {"data": registro}

        except Exception as e:
            logger.error(f"Error al obtener TipoDisponibilidad {id_registro}: {e}")
    ...
    def listar(
        ...
        """Lista tipos de disponibilidad con paginación y filtros"""
        try:
            if pagina < 1:
                pagina = 1
            if por_pagina < 1:
            ...
            if busqueda:
                where_clauses.append(
                    "(Codigo LIKE %s OR Nombre LIKE %s)"
                )
                termino = f"%{busqueda}%"
            ...
            # Contar total
            count_query = f"SELECT COUNT(*) AS Total FROM TipoDisponibilidad WHERE {where_sql}"
            ...
            logger.error(f"Error al listar TipoDisponibilidad: {e}")
    ...
    def listar_todos(
        ...
        try:
            ...
            logger.error(f"Error al listar todos TipoDisponibilidad: {e}")
            return {"error": f"Error interno al listar registros: {str(e)}", "status": 500}
    ...
    def actualizar(self, id_registro: int, datos: Dict) -> Dict:
        """
        Actualiza un tipo de disponibilidad existente.
        ...
        try:
            ...
            logger.error(f"Error al actualizar TipoDisponibilidad {id_registro}: {e}")
    ...
    def eliminar(self, id_registro: int) -> Dict:
        """
        Eliminación lógica de un tipo de disponibilidad.

        Validaciones:
            - Registro existe
            - No tiene motoristas activos asociados
        """
        try:
            existente = self._validar_existencia(id_registro)
            if not existente:
            ...
            if self._validar_tiene_motoristas_asociados(id_registro):
                return {
                    "error": "No se puede eliminar este tipo de disponibilidad porque tiene motoristas activos asociados",
                    "status": 409
                }
            ...
            return {"data": {"mensaje": f"Tipo de disponibilidad '{existente['Nombre']}' eliminado exitosamente"}}

        except Exception as e:
            logger.error(f"Error al eliminar TipoDisponibilidad {id_registro}: {e}")
